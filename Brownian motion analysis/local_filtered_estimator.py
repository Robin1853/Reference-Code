...
def lowpass_filter_with_reflection(signal, cutoff_ratio=0.1):
    """
    Denoising a multidimensional signal with FFT while preserving boundaries through reflection.

    signal: (T, D)
    cutoff_ratio: how many of the lowest frequency will be kept
    Return: denoised trajektory (T, D)
    """
    M, T, D = signal.shape
    all_smoothed = []
...
def KDTree_drift(local_point, tree, starts, steps, radius):
    ...
    weighted_drift = np.sum(neighbor_steps * weights[:, None], axis=0)
    return weighted_drift, neighbor_steps, neighbor_positions, weights

###estimated local drift field
def plot_drift_field(tree, starts, steps, radius=0.3, step=0.2):
    # Erzeuge 3D-Gitterpunkte im Bereich [-1,1]³
    x_vals = y_vals = z_vals = np.arange(-1, 1 + step, step)
    X, Y, Z = np.meshgrid(x_vals, y_vals, z_vals)
    est_drifts = []

    # Arrays zur Aufnahme der Driftvektoren
    U = np.zeros_like(X)
    V = np.zeros_like(Y)
    W = np.zeros_like(Z)
    ...
    for i in range(X.shape[0]):
        for j in range(X.shape[1]):
            for k in range(X.shape[2]):
                ...
                drift, _, _, _ = KDTree_drift(query, tree, starts, steps, radius)
    ...
    ax.set_ylim([-1, 1])
    ax.set_zlim([-1, 1])
    ax.set_title("Driftfeld geschätzt mit KDTree")
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_zlabel("z")
    plt.tight_layout()
...
def local_filtered_estimator(signal, true_drifts, grid_step, radius, ab_ratio):
    ###Cutoff optimization
    # Parameter
    ...
    est_drifts = plot_drift_field(tree, starts, steps, radius, grid_step)
